...
Plotly.d3.csv('../output_charts/full_query.csv', function (err, rows) {
  ...
  var schoolName = unpack(rows, 'name'),
    ...
    facultySalary = unpack(rows, 'faculty_salary'),
    expenditurePerStudent = unpack(rows, 'instructional_expenditure_per_fte'),
    tuitionRevenuePerStudent = unpack(rows, 'tuition_revenue_per_fte'),
  ...
  function getStateData(chosenVariable, chosenState, chosenCity) {
    ...
    function dataToList() {
      ...
      var currentText = "<b> Name: " + schoolName[i] + "</b>"
        ...
        + "<br><b> Out of State Tuition: </b>" + "$" + thousands_separators(tuitionOut[i])
        + "<br><b> Tuition in State: </b>" + "$" + thousands_separators(tuitionIn[i])
        + "<br><b> Expenditure per Student: </b>" + "$" + thousands_separators(expenditurePerStudent[i])
        + "<br><b> Tuition Revenue per Student: </b>" + "$" + thousands_separators(tuitionRevenuePerStudent[i]);
